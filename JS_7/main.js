...
function searchElement () { // Create an array of words that match the search value
    var partWord = search.value.toLowerCase();
    var arrResult = [];
    ...
    drawArrResult (arrResult);
    bindResultHandlers ();
    
}

function find (event) { // Customize the search button
    var wordToSearch = search.value;
    var googleLink = "https://www.google.com/search?q=" + wordToSearch + "&btnI";
    window.open(googleLink);

    console.log("Координаты кнопки: X:" + event.clientX + " Y: " + event.clientY );
}

function drawArrResult (arr) { // Draw a list of the array of words
    clearResult ();
...
function bindResultHandlers () { // Hang events for list items when "click" and "dblclick"
    for ( var i = 0; i < ul.childNodes.length; i++) {
        ul.childNodes[i].addEventListener("click", addValueForSearch);
        ul.childNodes[i].addEventListener("dblclick", addFavouriteValue);
        
    }
}

function clearResult() { // Сlear list
    while (ul.lastChild) {
        ul.removeChild(ul.lastChild);
    }   
}

function drawFavouriteList () { // Draw a list of the array of favourite words
    drawArrResult (favouriteList);
}

function addValueForSearch (event) { // Add an element to search when "click"
    var li = event.target;
...
function addFavouriteValue (event) { // Add a word to favorite list when "dblclick"
    var li = event.target;

    if (favouriteList.indexOf(li.innerHTML) == -1 ) {
        favouriteList.push(li.innerHTML);
    }
